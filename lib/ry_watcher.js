...
RYWatcher.prototype.handleMsg = function (type, data){
  ...
  switch(type){
    ...
    // PublishMessage
    case 3:
      // 49 的情况，官方是直接发送 QueryMessage 来拉取消息，注意每天24点消息会过期。
      var pos = 6 + data[6] + 2;
      var msgTargetId = data.slice(pos + 1, pos + 1 + data[pos]);
      ...
      if (data[0] === 51){
          ...
          var newResult = {
                fromUserId: self.userInfo.user_id,
                toUserId: String.fromCharCode.apply(null, msgTargetId),
                type: 1,
                groupId: '',
          ...
          self.log('===================== 📣   PublishMessage =====================');
          self.log('此类消息为空', newResult.fromUserId, result.content.toUTF8());
          self.expotMessage('PublishMessage', newResult);
          self.send(self.genMsg('PubAckMessage'));
      }
      break;
    // QueryAckMessage
